    ...
    stages {
        ...
        stage('Run Tests') {
            steps {
                ...
                sh 'npx cucumber-js --config cucumber.json tests/features/**/*.feature --require tests/step-definitions/**/*.ts --require tests/hooks/hooks.ts --format json:cucumber-report.json --format summary --format progress-bar'
            }
        }
